...
app.post('/api/attendees/liff', async (req, res) => {
  try {
    const { meetingId, lineUserId, name, position } = req.body;
    if (!meetingId || !lineUserId || !name) {
      return res.status(400).json({ error: 'Missing required fields' });
    }
    const { rows: users } = await pool.query('SELECT id FROM users WHERE line_user_id = $1', [lineUserId]);
    let userId;
    if (users.length > 0) {
      userId = users[0].id;
    } else {
      const { rows: newUser } = await pool.query(
        'INSERT INTO users (username, password, name, surname, position, role, line_user_id, allowed_meeting_ids) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id',
        [`line_${lineUserId}`, 'line_login', name, '', position || '', 'USER', lineUserId, JSON.stringify([])]
      );
      userId = newUser[0].id;
    }
    ...
    if (existing.length > 0) {
      await pool.query('UPDATE attendees SET status = $1, position = $2 WHERE id = $3',
        ['ACCEPTED', position || '', existing[0].id]);
    } else {
      await pool.query(
