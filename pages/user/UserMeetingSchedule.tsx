...
export const UserMeetingSchedule: React.FC = () => {
    const { meetings, attendees, users, user } = useApp();
    const [filterStatus, setFilterStatus] = useState<'ALL' | 'UPCOMING' | 'COMPLETED'>('ALL');

    if (!user) return null;

    // Filter only meetings the user belongs to
    ...
    const filtered = filterStatus === 'ALL'
        ? myMeetings
        : myMeetings.filter(m => m.status === filterStatus);
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                ...
                <p className="text-gray-500 mt-1 text-sm">ตารางการประชุมที่คุณเข้าร่วม</p>
            </div>
