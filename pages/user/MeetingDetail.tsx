                    ...
                    {activeAgenda ? (
                        <>
                            ...
                            <div className="p-6 overflow-y-auto flex-1">
                                ...
                                {/* General Meeting Documents Section (Inserted here for visibility) */}
                                <GeneralDocumentsSection meetingId={meeting.id} />

                                {/* Agenda Attachments (if any) */}
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-gray-400">
                            <GeneralDocumentsSection meetingId={meeting.id} isCenter={true} />
                        </div>
                    )}
                </div>
            </div>
...
const GeneralDocumentsSection: React.FC<{ meetingId: string, isCenter?: boolean }> = ({ meetingId, isCenter }) => {
                    ...
                    <div key={doc.id} className="flex items-center p-3 border border-gray-200 rounded-lg hover:bg-orange-50 hover:border-orange-200 transition-colors group">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex gap-2 text-xs text-gray-500 mt-0.5">
                                ...
                                {doc.url ? (
                                    <a href={doc.url} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline ml-auto flex items-center gap-1">
                                        เปิดลิงก์ <Download size={12} />
                                    </a>
                                ) : (
                                    <a href={`/api/documents/${doc.id}/download`} target="_blank" rel="noreferrer" className="text-green-600 hover:underline ml-auto flex items-center gap-1">
                                        ดาวน์โหลด <Download size={12} />
                                    </a>
                                )}
                            </div>
                        </div>
